...
class IMDBWrapper:
    ...
    def get_castlist(self, title):
        cast = []
        title = self.escape(title)
        sql = "select n.name from cast_info c, name n where n.id = c.person_id and c.movie_id = (select id from title where kind_id = 1 and title like '%{title:s}%' limit 1) and role_id in (1,3)".format(title=title)
        cursor = self.conn.cursor ()
        cursor.execute (sql)
        rows = cursor.fetchall ()
        for row in rows:
    ...
    def get_recommendation_randomlist(self, entities, userpref):
        return self.get_recommendation(self, entities, userpref, True)
    
    # pick one movie randomly
    def get_recommendation_onerandom(self, entities, userpref):
        movies = self.get_recommendation(self, entities, userpref, False)
    ...
    def get_recommendation(self, entities, userpref, doshuffle = False):
        ...
        if len(userpref) == 0:
            ...
            for key, value in userpref.iteritems():
                ...
                #    1.1 movie    - extract directors, actors actress -> -0.6, -0.3, 0, +0.3, +0.6
                    ...
                    personids = ""
                    sql = "select distinct GROUP_CONCAT(person_id) from cast_info c where c.movie_id = (select id from title where title like '%{title}%' and kind_id = 1 limit 1) and role_id in (1,3,15) limit 50".format(title=movietitle)
                    ...
                    if personids is not None and len(personids) > 0:
                        personids = "(" + personids + ")"
                        sql = "select distinct movie_id from cast_info where person_id in " + personids
                        cursor.execute(sql)
                        rows = cursor.fetchall ()
                        print sql
                        for row in rows:
        ...
        return self.get_movielist(entities, orderbyscore, doshuffle)
    
    def get_movielist(self, entities, orderbyscore = "", doshuffle = False):
        person_id_str = ""
        movie_id_str = ""
        director_id_str = ""
        genre_str = ""
        ...
        sql = "select title, production_year, (s.rating/3) + s.score + s.norm_popularity as totalscore from title t, yarb_score s  where t.kind_id = 1 and s.movie_id = t.id "
        ...
        if not doshuffle:
            if orderbyscore == "":
                sql += " order by production_year desc limit 100"
            else:
                sql += " order by totalscore desc, production_year desc limit 100"
