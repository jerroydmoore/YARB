...
class NLparser:
	...
	## TODO: implement this check in loops
	## This function will return a classifier for obvious keywords
	## Example: "recommend", "suggestion"
	def _checkPriorityClassifiers(self, leaf):
		x = self._getLeafTxt(leaf)
		if x == "recommend" or x == "recommendation" or x == "suggest" or x == "suggestion":
			return "recommend"
		return ""
	...
	def process(self, input_string):
		...
		if returnVar == False:
			...
			returnVar.add_entity(entityName, self._getTreeTxt(x))
		
		modifiers = ['popular', 'top rating', 'worst', 'latest', 'later', 'recent', 'older', 'newer', 'newest',
		'oldest', 'longer', 'longest', 'shorter', 'shortest']
		for x in modifiers:
			if x in input_string.lower():
				returnVar.add_entity2('modifier', x, False, True)
		...
		for i,x in enumerate(chunked):
			...
			if self._isTreeBranch(x):
				if x.node == "B-QUESTION":
					...
					returnVar = self._parse_command(chunked)
					returnVar.change_classifier('recommend')
					break
				elif x.node == 'REQ':
				...
				if nodetype == 'YES':
					returnVar = EntitySet("answer", self.vb)
					returnVar.add_entity("yesno", "yes")
					break
				elif nodetype == 'NO':
					returnVar = EntitySet("answer", self.vb)
					returnVar.add_entity("yesno", "no")
					break
				elif nodetype == 'HI':
	...
	def _parse_userPref(self, chktree):
		...
		return self._parse_entity_pairs(chktree, entSet)
